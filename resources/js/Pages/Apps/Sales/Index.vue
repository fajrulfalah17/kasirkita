<template>
  <Head>
    <title>Report Sales - Kasir Kita</title>
  </Head>
  <main class="c-main">
            ...
            <div class="card border-0 rounded-3 shadow border-top-purple">
              <div class="card-header">
                <span class="font-weight-bold"
                  ><i class="fa fa-chart-bar"></i> Laporan Penjualan</span
                >
              </div>
              <div class="card-body">
                    ...
                    <tbody>
                      <tr v-for="sale in sales" :key="sale.id">
                        <td class="text-center">
                          <button
                            @click.prevent="destroy(sale.details[0].id)"
                            v-if="hasAnyPermission(['transactions.delete'])"
                        ...
                        <template v-for="item in sale.details" :key="item.id">
                          <td>{{ item.product.title }}</td>
                          <td>
                            Rp. {{ formatPrice(item.product.sell_price) }}
                          </td>
                          <td>{{ item.qty }}</td>
                        </template>
                        <td class="text-end">
  ...
  setup() {
    ...
    const destroy = (id) => {
      Swal.fire({
        ...
        confirmButtonText: "Ya, Segera hapus!",
        
      }).then((result) => {
        if (result.isConfirmed) {
          Inertia.delete(`/apps/transactions/${id}`);
